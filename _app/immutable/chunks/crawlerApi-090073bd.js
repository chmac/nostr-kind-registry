...
`).map(s=>{const[e,a]=s.split(","),o=parseInt(e),c=new Date(a);return{kind:o,seen:c}}).sort((s,e)=>e.seen.getTime()-s.seen.getTime())},g=async()=>{const t=await fetch(`${n}/relays/relaysList.txt`);return t.status!==200?[]:(await t.text()).trim().split(`
